...
두 급수가 서로 다른 파일에 있다. 삭은 아직 한국 음력에서만 쓰여
rules/kr/astro.py 에 있고, 태양 황경은 나라와 무관해 core/astro.py 에 있다.
여기서는 rules/kr/astro.py 를 통해 둘 다 읽는다 — 그쪽이 태양 황경을
재수출하므로 호출 모양은 이동 전과 같다.
