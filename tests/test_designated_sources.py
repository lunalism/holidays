...
# 근거를 아직 확인하지 못한 항목. 전부 임시공휴일이고, 전부 국무회의 의결일·관보
# 게재일 미확인이다(각 항목의 source_todo 참조).
#
# 선거일은 여기 없다. election 8 건은 전부 「관공서의 공휴일에 관한 규정」 제2조를
# source 에 달고 있다. 2028-04-12 만 비어 있었는데 그건 성격이 달라서가 아니라
# KASI 응답에서 나중에 발견해 넣으면서 조문을 빠뜨린 것이었고, 채워서 해소했다.
#
...
# rules/kr/feed.py:35) 아래라 .ics 에 실리지 않기 때문이다.
#
