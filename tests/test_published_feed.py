...
def test_the_published_de_feed_shares_no_uid_with_the_other_published_feeds():
    """발행된 feeds/de.ics 의 UID 가 다른 여덟 발행본의 어떤 UID 와도 겹치지
    ...
    published = set()
    for other in (
        feed, jp_feed, kr_jp_feed, kr_only_feed, jp_only_feed, de_be_feed, de_by_feed, de_he_feed,
    ):
        published |= _published_uids(other.FEED_PATH)
...
def test_the_published_de_be_feed_shares_no_uid_with_the_other_published_feeds():
    """발행된 feeds/de_be.ics 의 UID 가 다른 여덟 발행본과 겹치지 않는가.
    특히 de.ics 와는 전국 공통 9 건이 같은 날 같은 항목이라, token 접두사
    de_be- 가 없으면 반드시 겹친다."""
    published = set()
    for other in (
        feed, jp_feed, kr_jp_feed, kr_only_feed, jp_only_feed, de_feed, de_by_feed, de_he_feed,
    ):
        published |= _published_uids(other.FEED_PATH)
...
def test_the_published_de_by_feed_shares_no_uid_with_the_other_published_feeds():
    """발행된 feeds/de_by.ics 의 UID 가 다른 여덟 발행본과 겹치지 않는가.
    de.ics 와는 전국 공통 9 건이, de_be.ics 와도 같은 9 건이 같은 날 같은 항목이라
    token 접두사 de_by- 가 없으면 반드시 겹친다."""
    published = set()
    for other in (
        feed, jp_feed, kr_jp_feed, kr_only_feed, jp_only_feed, de_feed, de_be_feed, de_he_feed,
    ):
        published |= _published_uids(other.FEED_PATH)
...
def test_the_published_de_he_feed_shares_no_uid_with_the_other_published_feeds():
    """발행된 feeds/de_he.ics 의 UID 가 다른 여덟 발행본과 겹치지 않는가.
    전국 공통 9 건은 de·de_be·de_by 와, Fronleichnam 은 de_by 와 같은 날 같은
    항목이라 token 접두사 de_he- 가 없으면 반드시 겹친다."""
    published = set()
    for other in (
        feed, jp_feed, kr_jp_feed, kr_only_feed, jp_only_feed, de_feed, de_be_feed, de_by_feed,
    ):
        published |= _published_uids(other.FEED_PATH)
