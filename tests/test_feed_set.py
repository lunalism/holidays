"""피드 코드 집합은 네 지점에서 같아야 한다.
    ...
    publish.yml 의 FEEDS         발행 워크플로가 도는 목록 — 여기 없으면 자동 발행에서 빠진다
...
본다. 집합 동치는 추이적이라 허브 하나면 넷이 전부 묶인다.

왜 마커를 붙이지 않나
--------------------

test_landing·test_readme 에도 집합 비교가 있다(feeds/ ↔ 랜딩 feed-data ↔
status.json, feeds/ ↔ README 구독 표). 그것들은 커밋된 산출물을 읽는 것이라
published_artifact 마커가 붙어 있고, 발행 워크플로는
`-m "not published_artifact"` 로 그 마커를 뺀다 — 규칙을 바꾼 PR 에서
발행본이 낡아 깨지는 것을 발행 자체가 막지 않기 위해서다(pyproject.toml).

여기서 보는 넷은 산출물이 아니라 발행의 **입력**이다. 발행 run 이 자기
입력의 누락을 스스로 잡으려면 이 테스트가 그 run 에서도 돌아야 한다. 그래서
마커가 없고, 그 결과 publish.yml 이 도는 테스트 집합이 이 파일만큼 넓어진다
— 이 테스트가 빨개지면 발행이 멈춘다. 의도한 것이다.

feeds/ 를 허브로 쓰지만 발행본의 내용은 읽지 않는다. 파일 이름만 센다.
"""
...
def _explain(name: str, got: set[str], hub: set[str]) -> str:
    ...
    덜 있는지가 갈려야 한다."""
    return (
        f"{name} 의 피드 집합이 feeds/ 와 다르다.\n"
        f"  {name} 에만 있음 (feeds/ 에 없음): {sorted(got - hub)}\n"
        f"  feeds/ 에만 있음 ({name} 에 없음): {sorted(hub - got)}"
    )


def test_every_rules_package_is_published_and_vice_versa():
    # rules/ 에만 있는 패키지는 고아다 — 어디에도 등록되지 않아 아무것도
    # 깨뜨리지 않고 아무것도 만들지 않는다. 발행본만 있고 패키지가 없으면
    # 재현할 수 없는 파일이 구독자에게 나가고 있는 것이다.
    got = _rules_packages()
    hub = _published_feeds()
    assert got == hub, _explain("rules/", got, hub)


def test_the_publish_workflow_feeds_match_the_published_feeds():
    # FEEDS 에서 빠진 발행본은 커밋된 채로 영원히 재발행되지 않는다. 규칙을
    # 바꿔도 낡은 파일이 그대로 서빙된다. FEEDS 에만 있는 코드는 생성 스텝이
    # rules.<코드>.feed 를 못 찾아 발행 전체를 깨뜨린다.
    got = _publish_feeds()
    hub = _published_feeds()
    assert got == hub, _explain("publish.yml FEEDS", got, hub)


def test_the_status_registry_matches_the_published_feeds():
