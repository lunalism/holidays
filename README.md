...
- `de.ics` — **16개 주 모두에서 유효한 날만.** 열여섯 주법의 교집합이고
  ...
  않습니다.
- `de_<주>.ics` — **전국 공통에 그 주의 주법 공휴일을 더한 상위집합.**
  `de.ics` 의 9건을 전부 포함하고 그 주에서만 쉬는 날이 얹힙니다.

**어느 걸 고르나.** 해당 주가 목록에 있으면 주 피드를 쓰십시오 — 그 주에서
실제로 쉬는 날 전부가 들어 있습니다. 주가 목록에 없거나 전국 어디서나
유효한 날만 필요하면 `de.ics` 를 쓰십시오. 둘을 같이 구독하면 9건이
중복됩니다.
