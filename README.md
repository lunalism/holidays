...
수집 대상이 아닙니다. 각 항목의 근거는 그 주 관보의 공포본이고(연방 관보
BGBl. 을 근거로 드는 항목은 `rules/de` 의 3. Oktober 와 그 미러뿐입니다),
서지(관보 호수·면·공포일·sha256)는 규칙 YAML 의 `source` 필드에 있습니다. sha256 은 받은 파일의 해시입니다. 파일이 결정적이지 않은
공포본(bgbl.de 처럼 요청마다 다시 만들어지는 PDF)은 규칙 YAML 머리 주석에
정의한 인용 면 이미지 스트림의 해시입니다.
