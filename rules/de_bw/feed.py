"""독일·바덴뷔르템베르크 주 공휴일 → core.ics.Event.
...
두 표, 한 계산
--------------------------------------------------------------------------
    solar_holidays.yaml       월·일 고정 7 건
    easter_holidays.yaml      부활절 오프셋 5 건
부활절은 python-dateutil 의 easter() 로 계산한다(rules/de/feed.py 의 근거).
Fronleichnam 은 부활절 +60(de_by·de_he·de_nw 와 같은 계산 경로). 일회성 표는 두지
않는다 — FTG § 1a(Reformationsfest 2017 한시)는 2017-11-01 에 실효해 발행 하한
(2020) 밖이다. 사례가 오면 de_be 의 로더를 그때 이식한다. key 규약은 그때를 위해
연도 접미사까지 지금부터 허용한다(_KEY_RE).

--------------------------------------------------------------------------
UID token 에 접두사를 단다
--------------------------------------------------------------------------
token 은 "de_bw-" + key 다. 주 피드 규약 {피드토큰}-{key} (docs/holiday_12.md §6).
key 는 전부 기존 확립값(공통 9 종은 de·여섯 주 피드와, heilige_drei_koenige·
fronleichnam·allerheiligen 은 de_by 와 같다)이라 접두사가 없으면 같은 날 같은 항목이
같은 UID 로 나간다. 함께 구독한 캘린더에서 같은 UID 는 서로를 덮어쓴다
...
피드는 12 건 전부 true 지만 그 사실도 나가지 않는다.

--------------------------------------------------------------------------
SUMMARY 는 BW 조문 표기에서 괄호 날짜를 뺀 것
--------------------------------------------------------------------------
YAML 의 name 그대로 — § 1 의 열거(Neufassung 1995 인쇄본)에서 "Erscheinungsfest
(6. Januar)" 와 "Allerheiligen (1. November)" 의 괄호 날짜를 뺐다. de.ics 가 BayFTG 의
서술부를 뺀 전례와 동형. 나머지 아홉 호는 조문 그대로다. 1월 6일은 조문 표기
"Erscheinungsfest" 가 SUMMARY 이고 통칭 Heilige Drei Könige 는 쓰지 않는다 — key 만
de_by 의 heilige_drei_koenige 를 재사용한다(token 은 내부 식별자). 3. Oktober 는
§ 1 에 없어 rules/de 의 표기 "Tag der Deutschen Einheit" 를 그대로 쓴다. 각 피드는
자기 근거 조문의 표기를 쓴다(de_be·de_he·de_hh·de_nw·de_sh 와 같은 결정).
"""
...
LAND_NAME_DE = "Baden-Württemberg"
...
# (_YYYY)만 허용. 날짜형(6_januar)은 숫자 시작이라, 움라우트(könige)는 charset 밖이라
# 막힌다 — ö→oe 가 heilige_drei_koenige 다. fullmatch() 인 이유도 같다(끝 개행이 새지
# 않게).
...
FEED_PATH = Path(__file__).resolve().parents[2] / "feeds" / "de_bw.ics"
