"""독일·라인란트팔츠 주 공휴일 → core.ics.Event.
...
일회성 표는 두지 않는다 — LFtG § 2 Abs. 2 에 일회성 수권이 있지만 발동은 2015 LVO
(GVBl. S. 433)의 2017-10-31 한 건뿐이고 발행 하한(2020) 밖이다. 사례가 오면 de_be 의
로더를 그때 이식한다. key 규약은 그때를 위해 연도 접미사까지 지금부터 허용한다(_KEY_RE).
...
피드는 11 건 전건 false + source_todo 지만 그 사실도 나가지 않는다.

--------------------------------------------------------------------------
SUMMARY 는 RP 조문 표기에서 관사와 괄호 날짜만 정리한 것
--------------------------------------------------------------------------
YAML 의 name 그대로 — § 2 Abs. 1 의 열거는 전 호가 "der …" 로 시작하고 Nr. 8·9·10 은
괄호 날짜를 단다. 관사 "der" 와 괄호만 빼고 나머지 자구는 그대로다: Nr. 5 "der Tag
Christi Himmelfahrt" → "Tag Christi Himmelfahrt", Nr. 7 → "Fronleichnamstag", Nr. 8
→ "Tag der Deutschen Einheit", Nr. 9 → "Allerheiligentag". Nr. 10 "der 1. und 2.
Weihnachtstag (25. und 26. Dezember)" 은 두 항목이 각자 자기 몫("1. Weihnachtstag" /
"2. Weihnachtstag")을 싣는다. BW 의 "Fronleichnam"·"Allerheiligen"·"Christi Himmelfahrt"
와 갈리는 것은 의도된 결과 — 각 피드는 자기 근거 조문의 표기를 쓴다(de_be·de_he·
de_hh·de_nw·de_sh·de_bw·de_ni 와 같은 결정).
"""
...
FEED_PATH = Path(__file__).resolve().parents[2] / "feeds" / "de_rp.ics"
