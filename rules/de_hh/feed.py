"""독일·함부르크 주 공휴일 → core.ics.Event.

직렬화는 하지 않는다. 그건 core/ics.py 다. 계약은 rules/de_he/feed.py 와 같다 —
...
일회성 표는 두지 않는다 — § 2 의 Sonderfeiertag 명령이 확인된 것이 없다. 사례가
오면 de_be 의 로더를 그때 이식한다. key 규약은 그때를 위해 연도 접미사까지
지금부터 허용한다(_KEY_RE).

--------------------------------------------------------------------------
UID token 에 접두사를 단다
--------------------------------------------------------------------------
token 은 "de_hh-" + key 다. 주 피드 규약 {피드토큰}-{key} (docs/holiday_12.md §6).
key 는 reformationstag 하나만 새것이고 나머지 9 종은 de·de_be·de_by·de_he 와
같다 — 접두사가 없으면 같은 날 같은 항목이 같은 UID 로 나간다. 함께 구독한
캘린더에서 같은 UID 는 서로를 덮어쓴다(rules/kr_only/feed.py 의 같은 절).
...
--------------------------------------------------------------------------
SUMMARY 는 함부르크 조문 표기
--------------------------------------------------------------------------
YAML 의 name 그대로 — § 1 의 열거(umwelt-online 현행판) 표기다. Nr. 7 의 괄호
날짜 "(3. Oktober)" 만 뺐다(de.ics 가 BayFTG 의 "der 3. Oktober als …" 서술부를
뺀 전례). Nr. 8 은 조문에 이름이 없어 SUMMARY 가 "31. Oktober" 다 — 통칭
Reformationstag 는 SUMMARY 에 쓰지 않고 source(DESCRIPTION)에만 둔다. 각 피드는
자기 근거 조문의 표기를 쓴다(de_be·de_he 와 같은 결정).
"""
...
# (_YYYY)만 허용. 날짜형(31_oktober)은 숫자 시작이라 막힌다 — Nr. 8 의 key 가
# reformationstag 인 이유다. fullmatch() 인 이유는 끝 개행이 새지 않게.
...
FEED_PATH = Path(__file__).resolve().parents[2] / "feeds" / "de_hh.ics"
