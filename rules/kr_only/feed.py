"""한국만 쉬는 날 피드 — kr 이벤트 중 jp 가 같은 날 쉬지 않는 것만.

정의는 차집합이다. kr 이벤트를 그대로 싣되, 그 날짜에 jp 공휴일이 하나라도
있으면 뺀다. 세 가지를 못 박는다.

1. 판정은 날짜 단위다. 이름이 달라도, token 이 달라도 날짜가 겹치면 뺀다.
   설날 연휴(2021-02-11)와 建国記念の日는 다른 공휴일이지만 같은 날이라
   이 피드에 없다. "같은 공휴일인가" 는 판정이고 판정은 근거를 요구하는데,
   이 피드가 답하는 물음은 "그날 상대국도 쉬는가" 뿐이다.

2. "상대국이 쉬지 않는다" 의 기준은 상대국 공휴일 집합뿐이다. 주말·근무
   형태·회사별 휴무는 개입하지 않는다. 토요일에 걸린 kr 공휴일도 그날 jp
   공휴일이 없으면 실린다.
...
--------------------------------------------------------------------------
token 에 kr_only-{cc}- 접두사를 단다. 발행된 세 피드의 UID 와 겹치면 안
된다 — 함께 구독한 캘린더에서 같은 UID 는 서로를 덮어쓴다. 접두사가 그
겹침을 구조적으로 막는다. cc 가 항상 kr 인데도 접두사에 넣는 것은 kr_jp 의
...
RANGE_END). 상대국 공휴일이 없는 구간을 실으면 그 구간 전체가 "한국만
...
def _tagged(event: ics.Event, cc: str) -> ics.Event:
    ...
    return dataclasses.replace(
        event,
        token=f"kr_only-{cc}-{event.token}",
        summary=f"[{cc.upper()}] {event.summary}",
    )


def events(start: date, end: date) -> list:
    """구간 안의 kr 이벤트 중 그날 jp 공휴일이 없는 것. 날짜 오름차순.

    상대국 날짜 집합은 jp 의 events() 전량에서 만든다 — 구간 밖의 날짜가
    섞여 있어도 자국 이벤트가 구간 안이라 결과는 같다. jp 의 events() 는
    인자를 받지 않는다(범위가 상수라서다).
    """
    other_days = {e.day for e in jp_feed.events()}
    kept = [_tagged(e, "kr") for e in kr_feed.events(start, end) if e.day not in other_days]
...
FEED_PATH = Path(__file__).resolve().parents[2] / "feeds" / "kr_only.ics"
