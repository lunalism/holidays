"""독일·슐레스비히홀슈타인 주 공휴일 → core.ics.Event.
...
일회성 표는 두지 않는다 — § 2 Abs. 2 의 수권에 따른 명령이 2019~2026 공포본에
없다. 사례가 오면 de_be 의 로더를 그때 이식한다. key 규약은 그때를 위해 연도
접미사까지 지금부터 허용한다(_KEY_RE).

--------------------------------------------------------------------------
UID token 에 접두사를 단다
--------------------------------------------------------------------------
token 은 "de_sh-" + key 다. 주 피드 규약 {피드토큰}-{key} (docs/holiday_12.md §6).
key 는 전부 기존 확립값(공통 9 종은 de·다섯 주 피드와, reformationstag 은 de_hh 와
같다)이라 접두사가 없으면 같은 날 같은 항목이 같은 UID 로 나간다. 함께 구독한
...
피드는 10 건 전부 true 지만 그 사실도 나가지 않는다.

--------------------------------------------------------------------------
SUMMARY 는 SH 조문 표기에서 날짜부·대시를 뺀 것
--------------------------------------------------------------------------
YAML 의 name 그대로 — § 2 Abs. 1 의 열거(공포본)에서 Nr. 7 "3. Oktober - Tag der
Deutschen Einheit -" 와 Nr. 8 "31. Oktober - Reformationstag -" 의 날짜부와 대시를
뺐다. de.ics 가 BayFTG 의 "der 3. Oktober als …" 서술부를 뺀 전례와 동형 — 명칭은
유지하고 날짜부만 제거한다. 나머지 여덟 호는 조문 그대로다. 각 피드는 자기 근거
조문의 표기를 쓴다(de_be·de_he·de_hh·de_nw 와 같은 결정).
"""
...
LAND_NAME_DE = "Schleswig-Holstein"
...
FEED_PATH = Path(__file__).resolve().parents[2] / "feeds" / "de_sh.ics"
