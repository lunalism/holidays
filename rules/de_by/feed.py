"""독일·바이에른 주 공휴일 → core.ics.Event.

직렬화는 하지 않는다. 그건 core/ics.py 다. 계약은 rules/de_be/feed.py 와 같다 —
...
일회성 표(designated_holidays.yaml)는 아직 두지 않는다 — 바이에른은 조문에
연도가 박힌 항목이 없고, 사례가 오면 de_be 의 로더를 그때 이식한다. key 규약은
그때를 위해 연도 접미사까지 지금부터 허용한다(_KEY_RE).

--------------------------------------------------------------------------
UID token 에 접두사를 단다
--------------------------------------------------------------------------
token 은 "de_by-" + key 다. 주 피드 규약 {피드토큰}-{key} (docs/holiday_12.md §6).
de.ics 는 접두사 없는 key 그대로를 token 으로 쓰고 de_be.ics 는 "de_be-" 를 쓰므로,
접두사가 없으면 Neujahr 같은 날 같은 항목이 세 피드에서 같은 UID 로 나간다.
...
--------------------------------------------------------------------------
SUMMARY 는 BayFTG 조문 표기
--------------------------------------------------------------------------
YAML 의 name 그대로, 움라우트 포함(Heilige Drei Könige). Art. 1 Abs. 1 Nr. 1 의
열거에서 정관사와 서술부를 뺀 것이다 — "der 1. Mai" → "1. Mai", "der 3. Oktober
als Tag der Deutschen Einheit" → "Tag der Deutschen Einheit", "Heilige Drei Könige
(Epiphanias)" → "Heilige Drei Könige". de.ics 도 BayFTG 표기를 쓰므로 전국 공통
9 건의 SUMMARY 는 de.ics 와 같다. token 은 ASCII 로 옮긴다(koenige).
"""
...
FEED_PATH = Path(__file__).resolve().parents[2] / "feeds" / "de_by.ics"
