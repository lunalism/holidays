"""독일·헤센 주 공휴일 → core.ics.Event.

직렬화는 하지 않는다. 그건 core/ics.py 다. 계약은 rules/de_by/feed.py 와 같다 —
...
일회성 표는 두지 않는다 — HFeiertagsG § 2 수권에 따른 명령이 확인된 것이 없다.
사례가 오면 de_be 의 로더를 그때 이식한다. key 규약은 그때를 위해 연도 접미사까지
지금부터 허용한다(_KEY_RE).

--------------------------------------------------------------------------
UID token 에 접두사를 단다
--------------------------------------------------------------------------
token 은 "de_he-" + key 다. 주 피드 규약 {피드토큰}-{key} (docs/holiday_12.md §6).
key 는 전부 기존 확립값(공통 9 종은 de·de_be·de_by 와, fronleichnam 은 de_by 와
...
SUMMARY 는 헤센 조문 표기
--------------------------------------------------------------------------
YAML 의 name 그대로 — § 1 Abs. 1 의 열거(umwelt-online 현행판)에서 정관사 der 를
뺀 것이다. de.ics·de_by.ics 와 다른 표기가 있다: Neujahrstag, Himmelfahrtstag,
Fronleichnamstag(de_by 는 BayFTG 표기 "Fronleichnam"), 1./2. Weihnachtstag.
맞추지 않는다 — 각 피드는 자기 근거 조문의 표기를 쓴다(de_be 의 Himmelfahrtstag
와 같은 결정, 2026-09-06 확인).
"""
...
# 하한·상한 정책은 kr·de·de_be·de_by 와 같다.
...
FEED_PATH = Path(__file__).resolve().parents[2] / "feeds" / "de_he.ics"
