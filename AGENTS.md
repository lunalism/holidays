...
**이 상태를 고치지 않는다.** 열려면 두 곳을 같이 바꿔야 하는데,
한쪽이 codex 플러그인의 버전별 캐시 파일이라 플러그인을 업데이트하면 조용히
되돌아간다. 되돌아간 것을 알아채는 방법도 없다 — 리뷰가 테스트를 돌린다고
믿는 상태에서 실제로는 안 돌리는 것이 지금보다 나쁘다.
다른 한쪽은 리뷰어에게 레포 쓰기 권한을 주는 것인데, 이 레포의 권한 방향
(자동 머지 금지·`main` 직푸시 금지·산출물은 기계만 씀)과 반대다.
