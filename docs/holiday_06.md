...
| 대상 | 줄 | 판정 |
|---|---|---|
| `PRODID`/`CALNAME`/`TZID` | 41-43 | **`CALNAME`/`TZID` 는 값만 kr, `PRODID` 는 두 피드가 값을 공유한다**(§4 의 `PRODID` 주). `ics.render()` 가 셋 다 인자로 받으므로 구조는 그대로 |
...
| 위치 | 지금 상태 |
|---|---|
| `rules/kr/feed.py:41-43` | `PRODID`/`CALNAME`/`TZID`. jp 가 바꾸는 것은 `CALNAME`/`TZID` 둘뿐이다. `PRODID` 는 그대로 쓴다 — 아래 주 |
...
**`PRODID` 주.** 전에 이 자리에 "jp 는 `//KO` 가 아닌 값이 필요"라고 적었다. 그 서술을 거둔다.

RFC 5545 에서 `PRODID` 는 캘린더가 아니라 **그 캘린더를 만든 제품**을 식별한다. 이 레포의 제품은 하나이고 같은 코드베이스·같은 파이프라인이 두 피드를 만든다. 피드마다 다른 `PRODID` 를 주면 다른 제품이 만들었다는 뜻이 되는데 사실이 아니다.

마지막 필드는 제품의 언어다. jp 피드의 산문도 한국어다 — `CALNAME` 이 `일본 공휴일` 이고 DESCRIPTION 서술문도 한국어다(§7). SUMMARY 만 일본어 원문인데 그것은 고유명사 표기 정책이지 피드 언어가 일본어라는 뜻이 아니다. `//JA` 로 바꾸면 일본어 피드를 기대한 구독자가 한국어 산문을 받는다.

기존 서술은 SUMMARY 표기 언어를 정하기 전에 쓴 것이고, 이번 세션에서 그 전제가 뒤집혔다.
...
**§4 와 §7 이 `PRODID` 에 대해 서로 다른 말을 하고 있었다.** §4 는 "jp 는 `//KO` 가 아닌 값이 필요"라고 적었고 §7 표는 `//KO` 인 값을 확정 사항으로 적었다. 머지 전 검토에서 발견되어 §4 쪽을 고쳤다 — `PRODID` 는 제품을 식별하고 이 레포의 제품은 하나이며, 마지막 필드인 언어도 jp 피드의 산문 언어가 한국어라 `//KO` 가 맞다. §4 의 기존 서술은 SUMMARY 표기 언어를 정하기 전에 쓴 것이다.

같은 결정이 §3-3 의 재사용 판정 표와도 어긋나 있었다. 그 표는 `PRODID`/`CALNAME`/`TZID` 를 묶어 "값만 kr" 이라고 적었는데, `PRODID` 는 두 피드가 값을 공유하므로 값조차 kr 전용이 아니다. 같은 검토에서 함께 고쳤다.
