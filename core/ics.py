...
# UID 네임스페이스가 확정되었는가.
#
# 확정했다. 이 값을 True 로 올린 커밋이 곧 그 기록이다.
#
# False 인 동안은 발행이 거부된다(rules/kr/feed.py 의 publish()). 확정 전에
# 나가는 것을 코드가 막는 장치였고, 확정 후에도 그대로 남긴다 — 다음 국가
# 피드나 네임스페이스를 다시 손대는 상황에서 같은 자리가 다시 쓰인다.
#
# 되돌리지 말 것. UID 는 이미 나갔다. 네임스페이스가 바뀌면 캘린더 앱이 같은
# 공휴일을 새 이벤트로 인식해 삭제 + 재생성으로 나타나고, SEQUENCE 를 올려도
# 수습되지 않는다(위 UID 절 참조).
#
...
def _sequences(pairs, published: dict) -> dict:
    ...
    ----------------------------------------------------------------------
    summarize_change() 와 기준이 다른 것은 의도한 것이다
    ----------------------------------------------------------------------
    그쪽은 (DTSTART, SUMMARY) 를 보고 이쪽은 날짜만 본다. 그래서 SUMMARY 만
    바꾼 재발행은 커밋 제목에 "변경 N 건"으로 찍히면서 SEQUENCE 는 0 그대로다.

    어긋난 것이 아니라 독자가 다르다. 커밋 제목은 사람이 히스토리를 훑는
    것이고, SEQUENCE 는 캘린더 클라이언트가 "다시 알림을 줄까"를 정하는 값이다.
    문구가 다듬어졌다는 것은 사람에게는 볼거리지만 구독자에게는 아니다.

    SUMMARY 변경으로 SEQUENCE 를 올리기 시작하면 앞으로 모든 문구 수정이 같은
    취급을 받는다. 대체공휴일 SUMMARY 에 원인을 붙인 31 건이 그 첫 사례였고,
    그때도 올리지 않기로 했다.
...
def summarize_change(previous: bytes, current: bytes) -> str:
    ...
    SUMMARY 를 포함해 세는 것은 의도한 것이다. _sequences() 는 날짜만 보므로
    문구만 바뀐 재발행은 SEQUENCE 가 안 올라가는데, 그 변경도 히스토리에는
    보여야 한다. 두 함수의 독자가 다르다 — 자세한 것은 _sequences() 참조.
    """
